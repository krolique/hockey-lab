...
def fetch_html(team_name, from_year, to_year, game_type,
    ...
    :type team_name: str
    :param from_year: begining year for the season
    :type from_year: int
    ...
    # values 2 or 3 for game tpye. Where 2 means regular and 3 playoffs.
    # Since we're wrapping this knowledge the rest of the module API
...
def extract_title(table_row):
    ...
    title_txt = table_row.find(name='div').txt
    title_txt = title_txt.replace('\n', '').replace('\t', '').strip()
    # every title contains this text, we don't really need the last bit
    # however if our assumption fails we should find out
    assert title_txt[20:] == '- Game By Game Stats'
    title_txt = title_txt[:20]
...
def process_html(html_text, game_type):
    """ """
    ...
    #
    #   1.  if the team does not have plaoff record they will return regular
    #       season game log
    ...
    if title != f'{from_year}-{to_year} {game_type}':
        raise RuntimeError('game log returned does not meet expectations')

    keys = extract_col_name(table_row(row[1]))
...
def generate_stream(team_name, from_year, to_year, game_type,
    ...
    for message in process_html(html_text=html):
